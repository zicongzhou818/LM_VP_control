function [phi_final1, phi_final2, phi_final3, T_local, T_final] = Img_Reg3D_LagMul_faithful_v3_mtlres_example2(T,R,N,mlyr)
T_original=T;
[mx,my,mz]=size(R);
...
%     ts = 1e-4;
    jd_bd = 0.2-lll*0.04;
    jd_bdm = 0.8;
    ts_tol = 5.0e-8;% tstep_ratio: stop when tstep less than tstep_ratio
    ts_up = 1.1;% tstep_up: magnification factor when ssd decreases
    ts_dn = 0.9;% tstep_down: reduction factor when doesn't ssd decrease
    ...
    T_old = T_old - min(T_old,[],"all");
    R_mtlres = imresize3(R, [sz sz sz]);
    R_mtlres = R_mtlres - min(R_mtlres,[],"all");
    ts = ts_init;
    ...
    while  (ts>ts_tol) && (ti<imax) && (r>r_tol) && (Jmin_comp>0)
        ...
        if better  % --(PS: Better holds for the first step in "while" loop)
            %% 3
            [Gx, Gy, Gz]=compute_gradient_vec_example2(X, Y, Z, T_local, R_mtlres, phi_m1,phi_m2,phi_m3, phi_comp1, phi_comp2, phi_comp3,omega_size,h,lll);
        end
        %% 1
