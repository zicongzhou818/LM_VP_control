function [Gx, Gy, Gz]=compute_gradient_vec_example2(X_init, Y_init, Z_init, T_local, R_mtlres, phi_m1,phi_m2,phi_m3, phi_comp1, phi_comp2, phi_comp3,omega_size,h,lll)
...
Gx = imgaussfilt3(Ix + IIx_IIIx, 1.2) ;
Gy = imgaussfilt3(Iy + IIy_IIIy, 1.2) ;
Gz = imgaussfilt3(Iz + IIz_IIIz, 1.2) ;

% Gx = imgaussfilt3(Ix + IIx_IIIx, 0.5*exp(1)-(lll*0.2));
% Gy = imgaussfilt3(Iy + IIy_IIIy, 0.5*exp(1)-(lll*0.2));
% Gz = imgaussfilt3(Iz + IIz_IIIz, 0.5*exp(1)-(lll*0.2));
